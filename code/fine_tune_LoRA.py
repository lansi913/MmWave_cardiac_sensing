...
backbone = load_filtered_state_dict(backbone,
                                    r'./Pretraing_weights.pth', map_location=device)
backbone.to(device)
backbone.eval()
...
# Due to institutional restrictions regarding patient privacy,
# the complete fine-tuning dataset is not publicly available.
# Access requests can be directed to the corresponding author,
# with an expected response time of up to 60 days.
# Then you can place the corresponding .pt files in their respective relative directories.
# Otherwise, you can bulid your own .pt fine-tuning dataset,
# which consists of a series of 30-second laballed displacement samples.
nor_pt_path = r'./Data/finetuning/Normal.pt'
# You need to set the correct directory path for the normal sample dataset file here.
...
PVC_pt_path = r'./Data/finetuning/PVC.pt'
# You need to set the correct directory path for the PVC sample dataset file here.
# You can further test other abnormal samples based on your own multi-task purpose.
arrhythmia_samples = []
arrhythmia_paths = {
    'PVC': PVC_pt_path,
}
...
os.makedirs('tune_results/data', exist_ok=True)

np.save(f'tune_results/data/results_{start_time}.npy', {
    'y_true': all_labels,
    'y_pred': all_preds,
...
plt.tight_layout()
plt.savefig('tune_results/LoRa_tune_percentage_' +
